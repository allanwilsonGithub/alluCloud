...
def maintain_local_backup_zips(source_dir,photos_source_dir):
    ...
    print "!AlluCloudTotalBackup started!(check log for more info)"
    logging.info("TotalBackup: Backup directory: %s"  % backup_source_dir)
    logging.info("TotalBackup: Zip is updated when changes are %s days old" % how_old_can_a_backup_be_before_recreating_zip)
    print "---------------------------"
    ...
    for dirpath in full_directory_list:
        ...
        if os.path.isfile(zip_file_path):
            ...
            if first_modification_after_zip_creation == -1:
                logging.info("TotalBackup: %s : No changes since zip was created" % dirname)
                ...
                logging.info("TotalBackup: New content has existed for %s days without being added to zip" % first_modification_after_zip_creation)
                print "Limit is set to %s days" % how_old_can_a_backup_be_before_recreating_zip
                logging.info("TotalBackup: Limit is set to %s days" % how_old_can_a_backup_be_before_recreating_zip)
                print "Time to recreate %s" % zip_file_path
                logging.info("TotalBackup: Time to recreate %s" % zip_file_path)
                ...
                logging.info("TotalBackup: %s : New content has existed for %s days without being added to zip. Limit is %s days so recreating zip in %s days" % (dirname,first_modification_after_zip_creation,how_old_can_a_backup_be_before_recreating_zip,countdown_to_recreate_zip))

        else:
            print "File wasn't found. creating... %s" % zip_file_path
            logging.info("TotalBackup: File wasn't found. creating... %s" % zip_file_path)
            create_zip_list.append(dirpath)
...
def zip_the_dirs(create_zip_list):
    ...
    for dir in create_zip_list:
        ...
        dirToCopy = dir + "//*.*"
        cmd = '"C://Program Files//WinRAR//WinRAR.exe" a -palluCloud -r -t \"%s\" \"%s\"' % (newRarName,dirToCopy)
        subprocess.call(cmd)



logging.info("TotalBackup: AlluCloudTotalBackup started")
maintain_local_backup_zips(backup_source_dir,photos_source_dir)
logging.info("TotalBackup: AlluCloudTotalBackup stopped")